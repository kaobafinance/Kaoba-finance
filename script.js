...
// --- CALCULO AUTOMÁTICO CALCULADORA ---
function calcular(){
...
perfilTitulares.addEventListener("change",()=>{
  if(perfilTitulares.value==="2"){
    ...
    salario2Div.style.display="block";
  } else {
    titular2Div.style.display="none";
    salario2Div.style.display="none";
  }
  calcularPerfil();
});
...
function calcularPerfil(){
  ...
  let plazoMax = perfilTipoVivienda.value==="local"?15:(perfilTipoVivienda.value==="segunda"?30:30);
  plazoMax = Math.min(plazoMax,75-maxEdad);
  ...
  let tipoRef = 0.028/12; // fijo 2.8% anual
  let n = plazoMax*12;
  ...
  if(yaTieneVivienda.checked){
    ...
    if(perfilTipoVivienda.value==="primera"){
      ...
      if(ahorro+capitalPosible<precio+gastos){
        perfilAviso.innerText=`Necesita aportar al menos ${formatMoneyPerfil(precio+gastos-(ahorro+capitalPosible))} adicionales para ser viable`;
      }
    } else if(perfilTipoVivienda.value==="segunda" || perfilTipoVivienda.value==="local"){
      let financiacionMax = 0.7*precio;
      gastos = (perfilTipoVivienda.value==="segunda"?precio*0.10:precio*0.10)+2500;
      if(capitalPosible>financiacionMax){
        perfilAviso.innerText=`El préstamo máximo permitido es 70% del precio. Necesita aportar mínimo ${formatMoneyPerfil((precio-financiacionMax)+gastos)} adicionales`;
      }
      capitalPosible = Math.min(capitalPosible,financiacionMax);
    }
  }
