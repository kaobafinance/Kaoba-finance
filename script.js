...
function calcularPerfil(){
  ...
  // reglas específicas: locales 15 años, segunda residencia 30 años
  if(perfilTipoVivienda.value==="local") plazoMax = Math.min(15,plazoMax);
  if(perfilTipoVivienda.value==="segunda") plazoMax = Math.min(30,plazoMax);
  ...
  // --- EURIBOR + diferencial 2% ---
  let tipoRef = 0.0415+0.02; // ejemplo, puedes actualizar dinámico
  tipoRef = tipoRef/12;
  ...
  if(yaTieneVivienda.checked){
    ...
    if(perfilTipoVivienda.value==="primera"){
      gastos = precio*0.10+2500; // IVA/ITP + otros
      capitalPosible = Math.min(capitalPosible, precio);
    } else if(perfilTipoVivienda.value==="segunda" || perfilTipoVivienda.value==="local"){
      gastos = precio*0.30 + 2500; // 30% entrada + gastos
      capitalPosible = precio*0.70; // financiable 70%
    }
    let ahorro = parseFloat(perfilAhorros.value)||0;
    capitalPosible = Math.min(capitalPosible, capitalPosible+ahorro-gastos); // considerar ahorro
  }
