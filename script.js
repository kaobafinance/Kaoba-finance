...
[perfilEdad1,perfilEdad2,perfilSalario1,perfilSalario2,perfilPagas,perfilAhorros,perfilDeuda,perfilOtroIngreso,perfilPrecio].forEach(el=>{
...
// -----------------------------
// OPERACIONES Y TARJETAS
// -----------------------------
function abrirOperacion(id){
  document.querySelectorAll('.card-content').forEach(cc=>{
    if(cc.id===id) cc.classList.toggle('open'); else cc.classList.remove('open');
  });
}
...
leadEnviar.addEventListener("click", ()=>{
  if(!leadNombre.value || !leadEmail.value || !leadConsent.checked){ alert("Completa todos los campos y acepta la política."); return; }
  ...
  if(perfilTitulares.value==="2"){ perfilDatos.push({label:"Edad titular 2",value:perfilEdad2.value},{label:"Ingreso mensual titular 2",value:perfilSalario2.value?formatMoney(perfilSalario2.value):"-"}); }
  ...
  consejos.forEach(c=>{doc.text("• "+c,margin,y); y+=15;});
  y+=10;
  doc.setDrawColor(0,63,139); doc.setLineWidth(1); doc.line(margin,y,555,y); y+=15;
