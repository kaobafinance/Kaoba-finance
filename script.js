...
function calcularPerfil(){
  ...
  const edad1=parseFloat(perfilEdad1.value)||0;
  const edad2=nTitulares===2?parseFloat(perfilEdad2.value)||0:0;
  ...
  const tipoRef=(parseFloat(document.getElementById("euriborValor").innerText.replace(',','.'))+2)/100/12;
  const n=plazoMax*12;
  ...
  if(yaTieneVivienda.checked){
    ...
    const impuestos=perfilTipoVivienda.value==="primera"?0.10*precio:
                      (perfilTipoVivienda.value==="segunda"||perfilTipoVivienda.value==="local"?0.7*precio:0);
  ...
  const ltv=yaTieneVivienda.checked?(capitalPosible/parseFloat(perfilPrecio.value)*100):0;
