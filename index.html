...
function calcularPerfil() {
  ...
  let plazoMax;
  const tipoVivienda = perfilTipoVivienda.value;
  if (tipoVivienda === "local") plazoMax = 15;
  else if (tipoVivienda === "segunda") plazoMax = 30;
  else plazoMax = Math.min(30, 75 - maxEdad);
  ...
  let tipoRef = (parseFloat(document.getElementById("euriborValor").innerText.replace(",", ".")) + 2.0) / 100 / 12 || 0.028 / 12;
  let n = plazoMax * 12;
  ...
  if (yaTieneVivienda.checked) {
    ...
    let financiacion = 0.7 * precio; // 70% máximo

    // Gastos extra
    let impuestos = tipoVivienda === "local" ? precio * 0.10 : tipoVivienda === "segunda" ? precio * 0.10 : precio * 0.10;
    gastos = (precio - financiacion) + impuestos + 2500;

    // Ajuste capital posible
    capitalPosible = Math.min(financiacion, capitalPosible);
  }
...
// --- EURIBOR ---
document.getElementById("euriborValor").innerText = "4.15"; // valor de ejemplo
